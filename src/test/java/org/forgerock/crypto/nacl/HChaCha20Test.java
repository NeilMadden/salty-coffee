...
public class HChaCha20Test {

    @DataProvider
    public Object[][] libsodiumTestVectors() {
    ...
    @Test(dataProvider = "libsodiumTestVectors")
    public void shouldMatchLibsodiumTestVectors(String keyHex, String inputHex, String expectedHex) {
        byte[] key = fromHex(keyHex);
        byte[] in = fromHex(inputHex);
        byte[] expected = fromHex(expectedHex);

        byte[] result = HChaCha20.apply(key, in);
