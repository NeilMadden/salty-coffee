...
final class Ed25519 {
    ...
    static byte[] sign(final byte[] message, final byte[] publicKey, final byte[] hashedPrivateKey) {
        ...
        MessageDigest digest = SHA512.getDigest();
    ...
    static class PrivateKey implements java.security.PrivateKey {
        ...
        PrivateKey(byte[] keyBytes) {
            if (keyBytes.length != SECRET_KEY_LEN) {
                throw new IllegalArgumentException("invalid Ed25519 private key");
            }
            this.keyBytes = keyBytes.clone();
            this.publicKey = scalarMultWithBaseToBytes(getHashedScalar());
        }
        ...
        byte[] getHashedScalar() {
            byte[] h = SHA512.hash(keyBytes, SHA512.HASH_LEN);
    ...
    static class PublicKey implements java.security.PublicKey {
        ...
        PublicKey(byte[] keyBytes) {
            if (keyBytes.length != PUBLIC_KEY_LEN) {
