...
final class Poly1305 {
    ...
    static final int MAC_TAG_SIZE_IN_BYTES = 16;
    static final int MAC_KEY_SIZE_IN_BYTES = 32;
    ...
    static byte[] compute(final byte[] key, byte[] data, int from, int to) {
    ...
    private static byte[] computeMac(final byte[] key, byte[] data) {
    ...
    static boolean verify(final byte[] key, byte[] data, byte[] mac) {
        byte[] computed = compute(key, data, 0, data.length);
        try {
            return Bytes.equal(computed, mac);
        } finally {
